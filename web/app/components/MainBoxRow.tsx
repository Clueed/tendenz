...
import { MainCardContent } from "./MainBoxRowSecond";
...
interface AnimationsConfigType {
  variants: Variants;
}

const primaryTransition: Transition = {
  duration: 1,
  type: "spring",
  when: "beforeChildren",
  staggerChildren: 0.3,
...
const secondaryTransition: Transition = {
  ...primaryTransition,
  delay: 1,
};

export const AnimationsConfig = createContext<AnimationsConfigType>({
  variants: {
    visible: { opacity: 1, transition: primaryTransition },
    transparent: { opacity: 0, transition: primaryTransition },
    unSlideRight: { opacity: 0, x: -10, transition: primaryTransition },
    slideRight: { opacity: 1, x: 0, transition: secondaryTransition },
  },
});
  ...
  return (
    <Accordion.Item value={entry.ticker} className="relative">
      <motion.div
        ...
        animate={{ x: x.get() }}
        className="relative p-3 mr-5"
      >
        <Accordion.Trigger>
          <MainCardTrigger entry={entry} expanded={expanded} />
        </Accordion.Trigger>
        <AnimatePresence>
          {expanded && (
        ...
        <CardBackground
          expanded={expanded}
          className="w-full h-full from-sky-4 to-indigo-6 -z-10"
        />
      </motion.div>
      <motion.div
      ...
      <CardBackground
        expanded={expanded}
        initial="unSlideRight"
        animated="slideRight"
        className="from-sky-5 to-indigo-7 -z-20 bg-gradient-to-br"
      />
    </Accordion.Item>
  );
}
